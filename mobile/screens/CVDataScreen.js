...
export default function CVDataScreen({ navigation }) {
  ...
  const fetchUploads = useCallback(async (isLoadMore = false, isRefresh = false) => {
    ...
    try {
      ...
      const response = await axios.get(`${getBaseURL()}/api/tenant/data/cv?page=${page}&limit=20&search=${searchQuery}`, {
  ...
  const handleExportCSV = useCallback(async () => {
    try {
      ...
      const fileUri = FileSystem.documentDirectory + 'cv_export.csv';
    ...
    if (uploads.length === 0) {
      return (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyMessage}>No commercial vehicle data found</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Commercial Vehicle Data</Text>
